...
def install_package(package_name: str) -> None:
	subprocess.run([INSTALL_PYTHON_PATH, '-m', 'pip', 'install', package_name])

def install_packages_from_requirements_file(path: str) -> None:
	subprocess.run([INSTALL_PYTHON_PATH, '-m', 'pip', 'install', '-r', path])
...
new_venv = venv.create(os.path.join(INSTALL_PATH, '.venv'), with_pip=True)
install_packages_from_requirements_file(os.path.join(INSTALL_PATH, 'requirements.txt'))
